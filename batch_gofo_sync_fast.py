...
# Unit prices (matching previous fixes)
PIECE_RATES = {
    'AAS Sorter 1': 0.095,
    'AAS Sorter 3': 0.095,
    'AAS Sorter 4': 0.095,
...
def sync_day(target_date_str):
    ...
    all_rows = []
    logging.info(f"⏳ Fetching {target_date_str} (1-hour chunks)")
    ...
    while current_start_dt <= overall_end_dt:
        current_end_dt = current_start_dt + timedelta(hours=1) - timedelta(seconds=1)
        ...
        while True:
            ...
            try:
                ...
                logging.error(f"Error in chunk {chunk_begin_str}: {e}")
                break
    ...
    def get_unit_price(name):
        return PIECE_RATES.get(name, 0.0)
...
def run():
    ...
    for ds in dates:
        count = sync_day(ds)
        logging.info(f"✅ Saved {count} rows for {ds}")
