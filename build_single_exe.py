...
def build_single_exe():
    ...
    cmd = [
        "pyinstaller",
        "--onefile",  # 单文件模式
        "--windowed",  # 无控制台窗口模式（可选，如果需要GUI模式）
        "--name", "InboundRecorder",  # 可执行文件名称
        "--icon", "in.ico",  # 使用指定图标文件
        "--add-data", "static;static",  # 添加静态文件目录
        "--add-data", "inbound.db;.",  # 添加数据库文件
        "--add-data", "DEPLOYMENT.md;.",  # 添加部署文档
        "--add-data", "DEPLOYMENT_FULL.md;.",  # 添加完整部署文档
        "--hidden-import", "openpyxl",  # 隐藏导入
        "--hidden-import", "pytz",  # 隐藏导入
...
def build_console_exe():
    ...
    cmd = [
        ...
        "--onefile",  # 单文件模式
        "--console",  # 显示控制台窗口
        "--name", "InboundRecorderConsole",  # 可执行文件名称
        "--icon", "in.ico",  # 使用指定图标文件
        "--add-data", "static;static",  # 添加静态文件目录
        "--add-data", "inbound.db;.",  # 添加数据库文件
        "--add-data", "DEPLOYMENT.md;.",  # 添加部署文档
