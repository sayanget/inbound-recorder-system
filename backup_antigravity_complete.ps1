...
Write-Host "========================================"
Write-Host "  Antigravity Complete Backup Tool"
Write-Host "========================================"
Write-Host ""
...
# Backup global .agent (using correct path)
...
if (Test-Path $globalAgentPath) {
    ...
    Write-Host "  - Global .agent not found at $globalAgentPath"
}
...
# Backup .gemini (using correct path)
...
if (Test-Path $geminiPath) {
    if ($SkipHistory) {
        ...
        $destPath = Join-Path $backupDir ".gemini"
        Write-Host "  - Copying .gemini directory (this may take a while)..."
    ...
    Write-Host "  - .gemini not found at $geminiPath"
}
Write-Host ""
