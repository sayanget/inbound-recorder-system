...
def inspect():
    ...
    # Get Meta
    url_meta = f'https://open.feishu.cn/open-apis/sheets/v3/spreadsheets/{spreadsheet_token}/metainfo'
    ...
    if res_meta.status_code == 200:
        ...
        if res_values.status_code == 200:
            ...
            print(f"\nValues in {target_id}: found {len(vals)} rows")
            for r in vals[:3]:
                print(f"  {r}")
        else:
            print(f"\nError reading {target_id}: {res_values.text}")
    else:
        print(f"Error reading meta: {res_meta.text}")
